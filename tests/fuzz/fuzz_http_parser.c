...
/* fuzz_http_parser - libFuzzer target for http parser parsing. Depends on: check, the module under test. */
int server_test_parse_chunks(const char **chunks, const size_t *lengths, int count,
                             char *method, size_t method_size,
                             char *path, size_t path_size,
                             char *body, size_t body_size);

/* SERVER_TEST seams in server.c (uv capture stubs for the response
 * helpers): the fuzzer never reaches the response path, so these are
 * link-only stubs. */
int server_test_uv_write(uv_write_t *req, uv_stream_t *stream,
                         const uv_buf_t bufs[], unsigned int nbufs,
                         uv_write_cb cb)
{
    (void)req; (void)stream; (void)bufs; (void)nbufs; (void)cb;
    return -1;
}
